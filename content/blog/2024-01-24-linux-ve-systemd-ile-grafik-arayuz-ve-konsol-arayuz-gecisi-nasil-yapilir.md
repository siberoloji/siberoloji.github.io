...
- right-sidebar
date: "2024-01-24T22:59:29Z"
excerpt: Bu yazımızda, systemd yönetim sistemini kullanan Linux dağıtımında grafik
  ve konsol arasında geçişler ve varsayılan açılış hedefini nasıl ayarlayabileceğimizi
  göreceğiz.
guid: https://www.siberoloji.com/?p=675
id: 675
...
<!-- /wp:code -->
  ...
  Öncelikle sistemimizde varsayılan olarak ayarlı açılış hedefini görelim. Bunun için "<code>systemctl get-default</code>" komutunu kullanıyoruz. 
...
<!-- /wp:code -->
  ...
  Sistemimizde ayarlanmış varsayılan açılış hedefinin grafik arayüz "<code>graphical.target</code>" olduğunu görebilirsiniz. Buradan çıkaracağımız sonuç, sistemimiz başladığında bütün servisler sırasıyla çalıştırılacak ve grafik arayüz ile masaüstü yöneticisine ulaşacaksınız.  
...
<!-- wp:code -->
<pre title="Açılışta komut satırına ulaşır." class="wp-block-code"><code lang="bash" class="language-bash">sudo systemctl set-default multi-user.target</code></pre>
<!-- /wp:code -->

<!-- wp:code -->
<pre title="Açılışta grafik arayüze ulaşır." class="wp-block-code"><code lang="bash" class="language-bash">sudo systemctl set-default graphical.target</code></pre>
<!-- /wp:code -->
