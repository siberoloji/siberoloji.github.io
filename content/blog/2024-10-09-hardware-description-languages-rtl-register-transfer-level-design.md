...
<!-- /wp:code -->
  ...
  This code snippet describes a simple register that is updated on the rising edge of a clock signal (<code>posedge clk</code>). If the reset signal is high, the register is cleared to zero; otherwise, it stores the value from <code>data_in</code>.
